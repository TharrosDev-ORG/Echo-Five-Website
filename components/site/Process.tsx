  ...
  return (
    <section id="process" className="pad-top-xl pad-bot-lg bg-paper">
      <div className="u-container">
        ...
        <Reveal delay={0.05}>
          <h2 className="t-h2 mt-8 max-w-[18ch] text-balance">{process.heading}</h2>
        </Reveal>
        <Reveal delay={0.1}>
          <p className="t-body mt-6 max-w-[58ch] text-pretty text-muted">{process.intro}</p>
        </Reveal>
        ...
        <ol className="relative mt-16">
          <div
            ...
            aria-hidden="true"
          />
          <div className="grid gap-y-10 lg:grid-cols-5 lg:gap-x-6 lg:gap-y-0">
            {process.stages.map((s, i) => (
              <Reveal as="li" key={`${s.key}-${s.name}`} delay={i * 0.08} className="relative flex gap-5 lg:block">
                {i < process.stages.length - 1 && (
                  <span
                    className="absolute left-7 top-14 -bottom-10 w-px bg-line lg:hidden"
                    aria-hidden="true"
                  />
                )}
                ...
                <div className="min-w-0 lg:mt-5">
                  ...
                  <h3 className="t-h3 mt-1">{s.name}</h3>
